	...
	<body>
			<form action='confirmRegistration.php' method='post'>
				Name: <input type="text" name="username" placeholder="enter name"> <br><br>
			 	...
			 	Repeat Password: <input type='password' name='passwordRepeated' id='passwordRepeated' size='25' /> <br><br>
			 	<input type="hidden" id="encryptedPasswordRepeated" name="encryptedPasswordRepeated">
				<button type ="submit" id='login' name="login2" value="send to database"> Create Account</button>
			</form>
			<a href="http://cs3750juan.epizy.com/login.php">Log in</a>
			<p>
				<?php
					$status = "";
					$userExists = "";
					...
					$userExists = $_SESSION["user_exists"];
					if($status == "failed")
					{
						echo "The passwords do not match, please try again";
					}
					if($userExists == "failed")
					{
						echo "That user exists please try a new user name";
					}
		...
		**/
		...
		document.getElementById('passwordRepeated').onkeyup = function(){
		 	var txt_string = document.getElementById('passwordRepeated').value;
		 	document.getElementById('encryptedPasswordRepeated').value = SHA256(txt_string);
		 	return false;
		}
		</script>
